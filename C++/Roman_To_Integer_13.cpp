...
public:
    ...
    int romanToInt(string s) {
        ...
        for (int i = 0 ; i < s.length() ; i++){ // We iterate through the string
            symbol_1 = charToValue(s[i]); // We call our function and convert it into integer
            if (i+1 < s.length()) { // This control structure is for i+1 symbol, and we make sure its in bounds
                symbol_2 = charToValue(s[i+1]); 
                if (symbol_1 >= symbol_2){ // LHS symbol is greater than RHS, like DC which means 600, we simply add the value to our answer
                    ans+=symbol_1;
                } 
                else
                {
